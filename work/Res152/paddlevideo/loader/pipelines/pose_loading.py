...
from .augmentations1 import Flip
...
class RandomRotate:
    ...
    def _rot(self,rot):
        ...
        rot = np.matmul(ry,rx,rz)
        
        return rot

    def rot(self,data_numpy):
        """
        data_numpy: C,T,V,M
        """
        data_numpy = data_numpy.transpose(3,1,2,0)
        C, T, V, M = data_numpy.shape
        data_numpy = data_numpy.transpose((1, 0, 2, 3)).reshape([T, C, V * M])  # T,3,V*M
        rot = np.random.uniform(-self.theta, self.theta,size=(3))
        rot = np.stack([rot, ] * T, axis=0)
        rot = self._rot(rot)  # T,3,3
        rot = rot[:,:2,:2]
        data_numpy = np.matmul(rot, data_numpy)
        data_numpy = data_numpy.reshape((T, C, V, M)).transpose((1, 0, 2, 3))
    ...
    def __call__(self, results):
        ...
        results['keypoint'] = self.rot(results['keypoint'])
...
class RandomFlip1:
    ...
    def __call__(self, results):
        ...
        if flip:
            results['keypoint'][...,0] = 400 - results['keypoint'][...,0]
...
class GeneratePoseTarget:
    ...
    def generate_heatmap(self, img_h, img_w, kps, sigma, max_values):
        ...
        if self.with_kp:
            ...
            for i in range(num_kp):
                ...
                # print("###########",heatmap.shape)
                # np.savetxt(str_+"_"+str(i)+".txt",heatmap,fmt="%.4f")
    ...
    def opt_flow(self,t,img_h,img_w):
        t_results = t.copy()
        t_results_list = []

        for i in range(t.shape[0]-1):
            t_list_temp = []
            for j in [0, 4, 7, 11, 14]:
                if np.max(t[i,:,:,j])<0.5 or np.max(t[i+1,:,:,j])<0.5:
                    mag = np.zeros((img_h, img_w))
                else:
                    current = t[i,:,:,j]*255
                    next = t[i+1,:,:,j]*255
                    flow = cv2.calcOpticalFlowFarneback(current.astype(np.int8), next.astype(np.int8), None, 0.5, 7, 15, 3, 5, 1.2, 0)
                    mag, ang = cv2.cartToPolar(flow[..., 0], flow[..., 1])
                t_list_temp.append(mag)
            t_np_temp = np.array(t_list_temp)
            
            t_np_temp = t_np_temp.transpose((1,2,0))
            t_results_temp = np.concatenate((t_results[i], t_np_temp), axis=-1)
            t_results_list.append(t_results_temp)
        return np.array(t_results_list)
    ...
    def gen_an_aug(self, results):
        ...
        res_array = np.stack(imgs)
        # res_array = self.opt_flow(res_array,img_h,img_w)
        return res_array
