...
def train_model(cfg,
   ...
   # 1. Construct model
    ...
    cfg.DATASET.train.keypoint_file = train_path
    cfg.DATASET.train.label_file = label_path
    ...
    best = 0.
    with LogWriter(logdir=osp.join(output_dir,"./log/scalar_test/train")) as writer:
        for epoch in range(0, cfg.epochs):
            ...
            writer.add_scalar(tag="loss", step=epoch, value=float(record_list['loss'].avg))
            writer.add_scalar(tag="train_top1", step=epoch, value=float(record_list['top1'].avg))
            ...
            def evaluate(best):
                ...
                writer.add_scalar(tag="val_top1", step=epoch, value=float(record_list['top1'].avg))
