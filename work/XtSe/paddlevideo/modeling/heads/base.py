...
class BaseHead(nn.Layer):
    ...
    def loss(self, scores, labels, valid_mode=False, **kwargs):
        ...
        """
        confusion_map = {
            3: 8, 7: 1, 14: 4, 21: 4, 12: 18, 15:19, 17:21, 20:16, 24:13, 25:18, 28:5
        }
        ...
        if len(labels) == 1:  #commonly case
            ...
            # confusion_label = copy.deepcopy(labels)
            # confusion_loss = 0
            # # batch
            # for i in range(confusion_label.shape[0]):
            #     # 如果label是小类，将label改成大类，并且计算小类和大类的focal loss
            #     # focal loss的x和y分别为: 2个类别的得分，小类label，小类为正样本
            #     if int(confusion_label[i]) in confusion_map:
            #         confusion_label[i] = confusion_map[int(labels[i])]
            #         x = paddle.unsqueeze(paddle.concat([scores[i][labels[i]], scores[i][confusion_label[i]]], axis=0), axis=0)
            #         focal_loss = fluid.layers.sigmoid_focal_loss(x, paddle.to_tensor([[0]], dtype='int32'), paddle.to_tensor(1, dtype='int32'))
            #         confusion_loss += paddle.sum(focal_loss[0], axis=0)
            ...
            # loss += self.cal_center_loss(scores[0], labels)
    ...
    def cal_center_loss(self, centers_distances, labels):
        loss = 0
        for i in range(labels.shape[0]):
            loss += centers_distances[i][labels[i]]
