...
class BaseHead(nn.Layer):
    ...
    def loss(self, scores, labels, valid_mode=False, **kwargs):
        ...
        """
        confusion_map = {
            3: 8, 7: 1, 14: 4, 12: 18, 15:19, 17:21, 20:16, 24:13, 25:18, 28:5
        }
        ...
        if len(labels) == 1:  #commonly case
            ...
            for i in range(confusion_label.shape[0]):
                ...
                if confusion_label[i] in confusion_map:
                    confusion_label[i] = confusion_map[labels[i]]
                    confusion_loss += fluid.layers.sigmoid_focal_loss(paddle.expand(scores[i][labels[i], confusion_label[i]], shape=[1,2]), [1])
                    
            losses = dict()
            if self.ls_eps != 0. and not valid_mode:  # label_smooth
                loss = self.label_smooth_loss(scores, confusion_label, **kwargs) + confusion_loss
            else:
                loss = self.loss_func(scores, confusion_label, **kwargs) + confusion_loss
