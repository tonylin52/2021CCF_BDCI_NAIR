...
def train_model(cfg,
    ...
    if num_folder != 0:
        cfg.DATASET.train["keypoint_file"] = osp.join(data_path, str(num_folder), "traindata.npy")
        cfg.DATASET.train["label_file"] = osp.join(data_path, str(num_folder), "trainlabel.npy")
        cfg.DATASET.valid["keypoint_file"] = osp.join(data_path, str(num_folder), "valdata.npy")
        cfg.DATASET.valid["label_file"] = osp.join(data_path, str(num_folder), "vallabel.npy")
    print(cfg.DATASET.train)
    print(cfg.DATASET.valid)
    ...
    for epoch in range(0, cfg.epochs):
        ...
        if validate and (epoch % cfg.get("val_interval", 1) == 0
            ...
            if save_best_flag:
                save(optimizer.state_dict(),
                    osp.join(output_dir, model_name + "_folder_"+str(num_folder)+ "_best.pdopt"))
                save(model.state_dict(),
                    osp.join(output_dir, model_name + "_folder_"+str(num_folder)+ "_best.pdparams"))
        ...
        if epoch % cfg.get("save_interval", 1) == 0 or epoch == cfg.epochs - 1:
            ...
            save(
                ...
                osp.join(output_dir,
                        model_name + "_folder_"+str(num_folder)+ f"_epoch_{epoch+1:05d}.pdopt"))
            save(
                model.state_dict(),
                osp.join(output_dir,
                        model_name + "_folder_"+str(num_folder)+ f"_epoch_{epoch+1:05d}.pdparams"))
