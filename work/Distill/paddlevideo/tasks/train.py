...
def train_model(cfg,
    ...
    mkdir(output_dir)
    writer = LogWriter(osp.join(output_dir,"./log/scalar_train_%s"%time.time()))


    # for p in model.state_dict():
    #     print(p,model.state_dict()[p])
    # save(model.state_dict(),"wanwan_best.pdparams")
    ...
    cfg.MODEL.head.num_classes = train_dataset.true_num_classes
    print("changing num_classes to %d"%train_dataset.true_num_classes)
    # 1. Construct model
    ...
    best = 0.
    num_iter = int(len(train_dataset)/batch_size) + 1
    for epoch in range(0, cfg.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if amp:
                ...
                #     if p.grad is None:
                #         # print("no grad: ", p.name)
                #         continue
                #     temp_grad = p.grad.numpy()
                #     # print(p.name, temp_grad.sum())
                #     writer.add_histogram(tag='%s_grad'%p.name,
                #                          values=p.grad,
                #                          step=epoch * num_iter + i)
                #     writer.add_histogram(tag='%s_param' % p.name,
                #                          values=p,
                #                          step=epoch * num_iter + i)
                    # if len(temp_grad.shape) > 2:
                    #     np.savetxt("temps/%s_epoch%d_iter%d.npy" % (p.name, epoch, i), temp_grad[0, 0, 0, ::])
                    # else:
                    #     np.savetxt("temps/%s_epoch%d_iter%d.npy" % (p.name, epoch, i), temp_grad)
            ...
            step = epoch * num_iter + i
            writer.add_scalar(tag="train/lr", step=step, value=optimizer.get_lr())
            writer.add_scalar(tag="train/loss", step=step, value=outputs['loss'])
            writer.add_scalar(tag="train/acc", step=step, value=outputs['top1'])
        ...
        def evaluate(best):
            ...
            for i, data in enumerate(valid_loader):
                ...
                res.append([label[0],np.argmax(classcore),outputs['top1'].item()])
            ...
            np.save(osp.join(output_dir,str(epoch)+cfg.METRIC.val_npy),res)
            ...
            writer.add_scalar(tag="val/loss", step=epoch, value=record_list['loss'].avg)
            writer.add_scalar(tag="val/acc", step=epoch, value=record_list['top1'].avg)
