...
class BaseHead(nn.Layer):
    ...
    def loss(self, scores, labels, valid_mode=False, **kwargs):
        ...
        if len(labels) == 1:  #commonly case
            ...
            if self.ls_eps != 0. and not valid_mode:  # label_smooth
                loss = self.label_smooth_loss(scores, labels, valid_mode, **kwargs)
            else:
                loss = self.loss_func(scores, labels,valid_mode, **kwargs)
            ...
            if self.ls_eps != 0:
                loss_a = self.label_smooth_loss(scores, labels_a,valid_mode, **kwargs)
                loss_b = self.label_smooth_loss(scores, labels_b,valid_mode, **kwargs)
            else:
                loss_a = self.loss_func(scores, labels_a,valid_mode, **kwargs)
                loss_b = self.loss_func(scores, labels_b,valid_mode, **kwargs)
    ...
    def label_smooth_loss(self, scores, labels,valid_mode, **kwargs):
        labels = F.one_hot(labels, self.num_classes)
        labels = F.label_smooth(labels, epsilon=self.ls_eps)
        labels = paddle.squeeze(labels, axis=1)
        loss = self.loss_func(scores, labels, soft_label=True,valid_mode=valid_mode, **kwargs)
        return loss
