...
class PoseDataset(BaseDataset1):
    ...
    def __init__(self,
        ...
        print("class_filter: ",self.class_filter)
        total_classes = list(range(num_classes))
        remain_classes = [i for i in total_classes if i not in class_filter]

        # 合并某些类别
        merge_cls_n = 0
        self.class2id = {}
        self.id2class = {}
        if len(class_merge) > 0:
            for i,group in enumerate(class_merge):
                for c in group:
                    assert self.class2id.get(c) is  None
                    self.class2id[c] = i
                    self.id2class[i] = "merge%d"%i
                    merge_cls_n += 1
        i = len(class_merge)
        for c in remain_classes:
            if self.class2id.get(c) is  None:
                self.class2id[c] = i
                self.id2class[i] = c
                i += 1

        self.true_num_classes = len(self.id2class)


        # self.class2id = {remain_classes[i]:i for i in range(len(remain_classes))}
        # self.id2class = {i:remain_classes[i] for i in range(len(remain_classes))}
        # self.true_num_classes = len(remain_classes)
        # print("calss merge: ", class_merge)
        # if class_merge:
        #     for c in self.class_filter:
        #         self.class2id[c] = self.true_num_classes
        #     self.true_num_classes += 1
        print("class to id map: ",self.class2id)
        print("id to class map: ", self.id2class)
        ...
        super().__init__(
            keypoint_file, label_file,conf_thres, pipeline, start_index=0, modality=modality,class_filter=class_filter, **kwargs)
    ...
    def load_npy_annotations(self):
        ...
        for i in range(traindata.shape[0]):
            ...
            if not self.test_mode:
                ...
                # if int(label[i]) in self.class_filter:
                #     # print(label[i])
                #     continue
                if label.shape[-1] == 1:
                    if self.class2id.get(int(label[i])) is None:
                        continue
                    res["label"] = self.class2id[int(label[i])]
                else:
                    res["label"] = label[i]
                # else:
                #     res["label"] = -1
