...
from ...utils.accuracy import (mean_average_precision, mean_class_accuracy,
                    mmit_mean_average_precision, top_k_accuracy)
# from .pipelines import Compose


class BaseDataset1(Dataset, metaclass=ABCMeta):
    ...
    def load_json_annotations(self):
        ...
        video_infos = json.load(self.ann_file)
    ...
    def dump_results(results, out):
        ...
        return json.dump(results, out)

    def prepare_train_frames(self, idx):
        if idx > len(self.video_infos)-1:
            print(idx)
        """Prepare the frames for training given the index."""
        results = copy.deepcopy(self.video_infos[idx])
        # print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&",results,self.modality)
